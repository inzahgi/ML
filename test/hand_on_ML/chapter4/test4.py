...
if __name__ == '__main__':
    ...
    print("line = 50", theta_best)

    X_new = np.array([[0], [2]])
    X_new_b = np.c_[np.ones((2, 1)), X_new]
    y_predict=X_new_b.dot(theta_best)
    print("line = 55", y_predict)
    ...
    lin_reg.fit(X, y)
    print("line = 66",lin_reg.intercept_, lin_reg.coef_)
    print("line = 67", lin_reg.predict(X_new))

    theta_best_svd, residuals, rank, s = np.linalg.lstsq(X_b, y, rcond=1e-6)
    print("line = 70", theta_best_svd)
